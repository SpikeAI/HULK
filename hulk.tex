...
\begin{equation}%
F \approx  \langle \frac{1}{2} \norm{\image_k - \dico \coef_{k}}_2^2 + \lambda\norm{\coef_{k}}_0 + \log(N) - \texttt{H}(\coef_{k}) \rangle_{k = 1 \ldots K}%
\label{eq:L0_cost_full}%
\end{equation}%
Where the entropy $\texttt{H}(\coef_{k}) = - \sum_i p_\dico(\hat{\coef_{k,i}}) \log p_\dico(\hat{\coef_{k,i}})$ acts as a proxy to measure the equalization of the sparse vector. %
