...
{\bf Homeostasis on Activation Probability and a quantitative evaluation of homeostatic strategies}: %
 {\sf (A)}~$18$ from the $441$ dictionaries learned for the two heuristics \texttt{EMP} and \texttt{HAP} and compared to the optimal homeostasis \texttt{HEH}. {\sf (B)}~Comparison of the free-energy during learning cross-validated over $10$ runs: The convergence of \texttt{HAP} is slightly better as \texttt{EMP} and is similar to the more demanding \texttt{HEH} homeostatic rule, demonstrating that this could be a good compromise for fast unsupervised learning.
\label{fig:HAP}}%
%\caption{
