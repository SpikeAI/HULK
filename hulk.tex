...
%$$ q(\coeff | \image; \dico) $$ tries to match
%$$ p(\coeff | \image; \dico) $$
...
% gradient descetn on $log[ p(x | \dico) ]:

% \frac{\partial }{\partial \dico_i } = \frac{-1}{2 \sigma^2} \frac{\partial }{\partial \dico_i }[(\image - \dico \coef)^T (\image - \dico \coef)] = \frac{-1}{2 \sigma^2} \hat{\coef} (\image - \dico \hat{\coef})]
...
% cf sparsity_4_dictionary_learning
...
% compatible with different sparse coding algorithms 
...
%: coding validated on sklearn

%: architecture to test zillions of parameters - better to spend time on this than to have only one model with zillions of parameters 
...
% Quantitative comparison : coordinate descent / matching pursuit / OMP / LARS / ...
