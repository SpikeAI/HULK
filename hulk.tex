...
{\sf (A)}~~Non-linear homeostatic functions $z_i, \forall i$ learned using Hebbian learning. These functions were computed for different homeostatic strategies ({\sf None}, {\sf OLS} or {\sf HEH}) but only used in {\sf HEH}. At convergence of {\sf HEH}, the probability of choosing any filter is independent. As a result, entropy is maximal, a property which is essential for the optimal representation of signals in distributed networks such as the brain.
{\sf (B)}~Effect of learning rate on the final cost as computed for the same learning algorithms but with different homeostatic strategies ({\sf None}, {\sf OLS} or {\sf HEH}).
{\sf (C)}~Effect of homeostatic learning rate on the final cost. This shows that {\sf HEH} is robust across a wide range of parameters.
...
\end{equation}%
We will coin this algorithm variant as Homeostasis on Activation Probability ({\sf HAP}). % derivation p.56N de 2018-01-09 HULK
%\subsection{Approximate homeostasis yields similar performance}
Following these derivations, we quantitatively compared ({\sf HAP}, {\sf EMP} and {\sf HAP}) to {\sf HEH}\seeFig{HAP}. This shows that while {\sf EMP} outperforms {\sf OLS}, {\sf HAP} proves to be closer to the optimal solution given by {\sf HEH}.
% effect of coding algorithm
In particular, we replicated in {\sf HAP} the result of Sandin that while homeostasis was essential in improving unsupervised learning, the coding algorithm (MP versus OMP) mattered relatively little.
...
{\bf Comparative evaluation of homeostatic strategies}: %
 {\sf (A)}~10 from the 324 dictionaries learned for the 3 algorithms ({\sf HEH}, {\sf EMP}, {\sf HAP}) and compared to the simple normalisation ({\sf None}). {\sf (B)}~Comparison of the free-energy during learning: The convergence velocity of {\sf HAP} is slightly better as {\sf EMP} and is similar to the more demanding HEH homeostatic, demonstrating that this could be a good compromise for fast unsupervised learning.
\label{fig:HAP}}%
%\caption{
