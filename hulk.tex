...
%   The authors propose a new adaptive homeostatic algorithm for the unsupervised
%   learning of visual features. The proposed method introduces a new regulation
%   process between learning and coding, called homeostasis. Homeostasis is also
%   optimized by a heuristic on the probability of activation of neurons, that
%   allows to implement a more rapid unsupervised learning algorithm while keeping a
%   large part of its effectiveness. 
%
%   Although the authors claim that their approach is new, they also point out that
%   their current method is an extension of a previous work from about nine years
%   ago [19], so the use of homeostasis in this context cannot be considered as "new".
%
%   The introduction is too large and there are too many "in particular" things (it
%   must be rewritten in a meaningful way). Some relevant parts of the explanation
%   are presented in a 53-pages annex that does not explain things pretty well. Some
%   points of the text are highlighted meaninglessly (for example, different colors
%   for words from the same concept).
%
%   The manuscript is too much confuse, there is no clear division among the main 
%   topics. Everything is explained as a single section, making the text unreadable
%   and hard to understand. It seems like related works, proposed approach,
%   experimental results and some conclusions are all merged together in a single
%   section, instead of being presented separately. Also, the proposed approach
%   seems to be a merge of already existent operators, so it might be called
%   anything but new.
%
%   The proposed approach also seems to be compared only with some variants of its
%   own operators from the literature, so there is no way to access how the proposed
%   method fits the current state-of-the-art for the application.
%
%   Minor Reviews
%
%   Introduction - last paragraph
%
%   Once some readers might opt to print the paper, "link" in "including the Annex
%   at this link" should be replaced by the actual link.
%
%   P.5 - C.
%   "replacing line 8 in the learning lgorithm" -> "replacing line 8 in the learning
%   algorithm"
%
%----------------------------------------------
%REVIEW NO. 2
%
%Comments to the authors:
%   This paper deals with a very interesting topic, well suited for IJCNN.
%   I have no doubt that authors have a deep competence on the treated topics.
%   However the paper is difficult to read, not for the use of English language, but 
%   for the overall organization of the paper and the way algorithms are explained.
%   To be fully understood the average reader is forced to download and read a set 
%   of referenced papers, not only for catching details.
%
%   Explanation on the variants are useful and interesting, but the paper lacks of a 
%   rigorous problem formulation and a precise description of the used data sets.
%   Could you provide a comprehensive, rigorous and precise pseudocode for the HAP 
%   procedure, improving the one describing HEH?
%
%   Please, avoid to use colors in any text excerpt.
%
%   The use of HAP in training CNN is explained just in the conclusion section (!), 
%   without providing any detail useful for experiment replication.
%
%   Please describe better this point in a proper section.
%
%----------------------------------------------
%REVIEW NO. 3
%
%Comments to the authors:
%   A biologically-plausible sparse Hebbian learning algorithm (combined with a
%   simplified homeostasis rule) is applied convincingly in a single layered
%   convolutional neural network for sparse coding.
%=================================================================
\firstpage{1} 
\makeatletter 
